...
## فاز چهارم — بخش اتوماسیون (Socket)

یک اسکریپت Python به عنوان بخش اختیاری فاز چهارم پیاده‌سازی شد که فایل `.pcapng` را به عنوان ورودی دریافت کرده و پکت‌های HTTP را پارس و خلاصه‌ای از آن‌ها را در ترمینال چاپ می‌کند.

### نحوه اجرا

```bash
python Socket_Programing.py Phase1_Capture.pcapng
```

### رویکرد پیاده‌سازی

به جای استفاده از کتابخانه‌های Python مانند `scapy` یا `pyshark` که نیاز به نصب جداگانه دارند، از ابزار `tshark` — نسخه command-line ویرشارک — به عنوان موتور پارس استفاده شد. اسکریپت با `subprocess` فرآیند tshark را فراخوانی می‌کند، فیلد‌های مورد نظر (IP مبدا و مقصد، پورت‌ها، متد HTTP و کد وضعیت) را با جداکننده `|` استخراج کرده و پس از پردازش، درخواست‌ها و پاسخ‌های HTTP را به صورت جداگانه در ترمینال نمایش می‌دهد.
...
HTTP Requests:
----------------------------------------------------------------------
Frame 5 | 192.168.1.6:3695 -> 188.114.98.0:80 | GET http://example.com/

HTTP Responses:
----------------------------------------------------------------------
Frame 8 | 188.114.98.0:80 -> 192.168.1.6:3695 | Status: 200 OK

Done.
```

فایل اسکریپت در مسیر [`Phase4/Socket/Socket_Programing.py`](Phase4/Socket/Socket_Programing.py) موجود است.
...
- اسکریپت اتوماسیون: [`Phase4/Socket/Socket_Programing.py`](Phase4/Socket/Socket_Programing.py)
